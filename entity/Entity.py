...
class Entity(pygame.sprite.Sprite):
    def __init__(self, game, name, position):
        ...
        self.image = pygame.Surface((226, 222), pygame.SRCALPHA, 32).convert_alpha()
    ...
    def animate(self):
        ...
        if not self.isWithBat:
            ...
            if self.isThrowing:
                ...
                self.image.blit(self.imageCollectionThrowing[int(self.throwTickFrame)], self.imageCollectionThrowing[int(self.throwTickFrame)].get_rect(center = (226/2, 222/2)))
                ...
                self.image.blit(self.imageCollection[int(self.tickFrame)], self.imageCollection[int(self.tickFrame)].get_rect(center = (226/2, 222/2)))
        else:
            if self.isThrowing:
                self.throwTickFrame = (self.throwTickFrame + 0.5)
                self.batTickFame = (self.batTickFame + 0.2) % 12
                self.image.blit(self.imageCollectionBatThrowing[int(self.batTickFame)%2], self.imageCollectionBatThrowing[int(self.batTickFame)%2].get_rect(center = (226/2, 222/2)))
                if int(self.throwTickFrame) == 3:
                    self.throwTickFrame = 0
                    self.isThrowing = False
            else:
                self.batTickFame = (self.batTickFame + 0.2) % 12
                self.image.blit(self.imageCollectionBat[int(self.batTickFame)], self.imageCollectionBat[int(self.batTickFame)].get_rect(center = (226/2, 222/2)))
        
        if self.isProtected:
            self.tickFrame = (self.tickFrame + 0.2) % 18
            self.image.blit(self.imageCollectionProtected[int(self.tickFrame)], self.imageCollectionProtected[int(self.tickFrame)].get_rect(center = (226/2, 222/2)))

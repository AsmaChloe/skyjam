...
class Entity(pygame.sprite.Sprite):
    def __init__(self, game, name, pickaxe_type, position, playerStateManager):
        super().__init__()
        self.game = game
        self.name = name
    ...
    def animate(self):
        ...
        if self.playerStates.isDynamiteStarting or self.playerStates.isDynamiteDuring or self.playerStates.isDynamiteEnding:
            self.image = pygame.Surface((219, 630), pygame.SRCALPHA, 32).convert_alpha()
            if self.playerStates.isDynamiteStarting:
                ...
                if int(self.dynamiteTickFrame) == 2:
                    self.dynamiteTickFrame = 0
                    self.playerStates.isDynamiteStarting = False
                    self.playerStates.isDynamiteDuring = True
                    self.game.sound_player.rocket_channel.play(self.rocketMan)
            elif self.playerStates.isDynamiteDuring:
                ...
                if int(self.dynamiteTickFrame) == 10:
                    ...
                    self.playerStates.isDynamiteEnding = False
        else:
            self.image = pygame.Surface((290, 320), pygame.SRCALPHA, 32).convert_alpha()
            if self.hasLost:
                self.image.blit(
                ...
                if self.playerStates.isEvolvingPickaxe:
                    ...
                    if int(self.evolutionTickFrame) == 17:
                        self.playerStates.isEvolvingPickaxe = False
                        self.evolutionTickFrame = 0
                        self.currentPickaxeType = self.game.pickaxe_type
                else:
                    if self.playerStates.isDynamite:
                        ...
                        if self.side:
                            ...
                            self.image.blit(surf, surf.get_rect(center = (290/2 - 35, 320/2)))
                    if not self.playerStates.isWithBat:
                        #animation lancée
                        if self.playerStates.isThrowing:
                    ...
                    if self.playerStates.isProtected:
    ...
    def throw(self):
        self.throwTickFrame = 0
        self.playerStates.isThrowing = True
    ...
    def update(self):
        ...
        if not self.playerStates.isEvolvingPickaxe and keys[pygame.K_q] and self.checkBound(True):
            self.flipAnimation(True)
            self.rect.left -= self.xSpeed
                
        if not self.playerStates.isEvolvingPickaxe and keys[pygame.K_d] and self.checkBound(False):
